...
const GLOBAL_CONFIG = {
    ...
    IS_GLOBAL_PRESALE_ENABLED: false,
...
export function cartLinesDiscountsGenerateRun(input) {
    ...
    GLOBAL_CONFIG.IS_GLOBAL_PRESALE_ENABLED = input.cart.lines.some(line => {
        const value = line.merchandise?.product?.isGlobalPreSaleEnabled?.value;
    ...
    console.log(">>> GLOBAL PRESALE ENABLED: ", GLOBAL_CONFIG.IS_GLOBAL_PRESALE_ENABLED);
    ...
    for (const line of lines) {
        ...
        if (product?.productType === GLOBAL_CONFIG.PRODUCT_TYPE && !hasPickyBundle) {
            try {
                ...
                const preSaleExtraTierConfig = safeParseJson(product.presaleExtraTierConfig?.value || null);
...
function calculateDiscounts(lines, crafterBundles = [],configuration, totalQuantity) {
    ...
    if(GLOBAL_CONFIG.IS_PRESALE_ENABLED && GLOBAL_CONFIG.IS_GLOBAL_PRESALE_ENABLED) {
    ...
    /*
    >>>CrafterBundles{
        "alen-r-1-bedroom-home-smart-bundle": {
          "items": [
            {
            ...
            {
              "product": "gid://shopify/Product/7246121500785",
              "handle": "alen-breathesmart-35i",
              "amount": 30,
              "preSaleAmount": 30,
              "type": "percentage"
            }
    ...
    */
      ...
      const bundleCandidates = Object.entries(crafterBundles).flatMap(([discountKey, bundleData]) => {
        try {
            // --- Paso 1: contar cuántos bundles completos se pueden formar ---
            const requiredCounts = bundleData.items.reduce((acc, item) => {
                acc[item.handle] = (acc[item.handle] || 0) + 1;
                return acc;
            }, {});

            console.log(">>> requiredCounts ", JSON.stringify(requiredCounts));
    
            const availableCounts = bundleData.ids.reduce((acc, idData) => {
                acc[idData.handle] = (acc[idData.handle] || 0) + idData.qty;
                return acc;
            }, {});

            console.log(">>> availableCounts ", JSON.stringify(availableCounts));
    
            const maxBundles = Math.min(
                ...Object.keys(requiredCounts).map(handle => 
                    Math.floor((availableCounts[handle] || 0) / requiredCounts[handle])
                )
            );

            console.log(">>> maxBundles ", maxBundles);
    
            if (maxBundles === 0) return [];
    
            // --- Paso 2: duplicar receta "items" según bundles disponibles ---
            let remainingItems = [];
            for (let i = 0; i < maxBundles; i++) {
                remainingItems = remainingItems.concat(bundleData.items.map(item => ({ ...item })));
            }

            console.log(">>> remainingItems ", JSON.stringify(remainingItems));
    
            // --- Paso 3: tu código original usando remainingItems normal ---
            const operations = [];
            let amount = 0;
            ...
            for (const idData of bundleData.ids) {
                ...
                if (itemIndex !== -1) {
                    ...
                    if (GLOBAL_CONFIG.IS_PRESALE_ENABLED && preSaleExtraTierConfig && GLOBAL_CONFIG.IS_GLOBAL_PRESALE_ENABLED) {
                        if(item.preSaleAmount <= 0) continue;
                        GLOBAL_CONFIG.DEFAULT_DISCOUNT_PREFIX = '[DC - PB] ';
                        amount = item.type === "percentage"
                        ? Math.min(Number(item.preSaleAmount), 100)
                        : Number(item.preSaleAmount);
                    } else {
    ...
    const candidates = lines.reduce((acc, line) => {
        ...
        try {
            ...
            if(isTierDiscount) {
                ...
                if(GLOBAL_CONFIG.IS_PRESALE_ENABLED && line.isPreSaleProduct.value.toString() == 'true' && GLOBAL_CONFIG.IS_GLOBAL_PRESALE_ENABLED) {
                    ...
                    discountToApply = Number(originalDiscount);

                    //discountCopy = `${discountCopy} ${originalDiscount}% OFF + ${GLOBAL_CONFIG.DEFAULT_PRE_SALE_PREFIX} ${preSaleExtraTierConfig}% OFF`;
                    discountCopy = `${discountCopy} ${originalDiscount}% OFF + ${GLOBAL_CONFIG.DEFAULT_PRE_SALE_PREFIX}% OFF`;
                } else {
                ...
                if(GLOBAL_CONFIG.IS_PRESALE_ENABLED && line.isPreSaleProduct.value.toString() == 'true' && GLOBAL_CONFIG.IS_GLOBAL_PRESALE_ENABLED) {
                    ...
                    if(discountType === 'amount') {
                        discountToApply = discountToApply * line.quantity;
